...
const PRESENTER_NOTES: Record<number, { timing: string; en: string[]; jp: string[] }> = {
  ...
  22: {
    ...
    en: [
      ...
      "Prompt injection — EchoLeak hit Copilot in June 2025 (CVSS 9.3). ForcedLeak hit Salesforce Agentforce in September (CVSS 9.4).",
      "Cross-border data exposure — LayerX 2025: 77% of employees have pasted data into generative AI as part of their work.",
      "Supply chain — April 2026 saw three simultaneous npm / PyPI / Docker Hub campaigns targeting AI tooling.",
      "API key leakage — one bad commit can mean millions of yen in unauthorized usage overnight.",
      "Shadow AI — employees using personal ChatGPT for work data. DLP doesn't catch it.",
      "Every one of these has a concrete response. Next 3 slides.",
    ],
    jp: [
      "AI導入の5つの脅威。",
      "エージェント乗っ取り — EchoLeak（Copilot 2025年6月、CVSS 9.3）、ForcedLeak（Salesforce 9月、CVSS 9.4）。",
      "プロバイダーへの情報流出 — LayerX 2025年調査：社員の77%が業務でAIにデータを貼り付け。",
      "サプライチェーン — 2026年4月、npm・PyPI・Docker Hubで同時攻撃。",
      "APIキー流出 — 一晩で数百万円の不正利用。",
      "シャドーAI — 個人ChatGPTで業務処理。DLPも効かない。",
      "5つすべてに、具体的な対応があります。",
    ],
  },
  23: {
  ...
  26: {
    ...
    en: [
      ...
      "Every vendor cost — VPS, keys, monitoring, certifications — is pass-through. MOTTO adds no margin.",
      "Security is a posture, not a product. As much as you need, transparently priced.",
    ],
    jp: [
      "セキュリティセクションの締めくくり。",
      "VPS、鍵、監視、認証 — すべて実費パススルー。マージン無し。",
...
export default function PresentationPage() {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full pt-12">
        ...
        {/* ===== SLIDE 21: SECURITY DIVIDER ===== */}
        <SlideWrapper active={slideIndex === 21} transition="slide" direction={direction}>
          <section className="h-full flex flex-col items-center justify-center px-8">
            <div className="flex flex-col items-center">
              <div className="inline-flex items-center gap-2 mb-6 px-3 py-1 rounded-full border border-[#B8860B]/20 bg-[#B8860B]/8 text-[#B8860B] text-xs font-medium uppercase tracking-wider">
                09 — セキュリティの構え / Security stance
              </div>
              <Shield className="size-14 text-[#B8860B] mb-6 opacity-80" />
              <h2 className="text-5xl sm:text-6xl font-bold text-center leading-tight">
                <span className="gradient-text">本当のリスクと、対応。</span>
              </h2>
              <p className="text-lg text-[#86868B] mt-4 text-center max-w-2xl">
                The threats facing every AI deployment in 2025–2026, and how we respond.
              </p>
              <p className="text-sm text-[#86868B]/70 mt-2 text-center max-w-xl">
                3つの層・5つの脅威・3つの対応 — 順に見ていきます。
              </p>
            </div>
          </section>
        </SlideWrapper>
          ...
          <section className="h-full flex flex-col items-center justify-center px-8 overflow-y-auto">
            <div className="flex flex-col w-full max-w-6xl">
              <div className="shrink-0 pb-3">
                <div className="inline-flex items-center gap-2 mb-3 px-3 py-1 rounded-full border border-red-200 bg-red-50 text-red-600 text-xs font-medium uppercase tracking-wider">
                  脅威 / The threats
                </div>
                <h2 className="text-2xl sm:text-4xl font-bold leading-tight">
                  AI導入の <span className="text-[#B8860B]">5つの脅威</span>
                </h2>
                <p className="text-sm text-[#86868B] mt-1.5 italic">
                  Five concrete threats every AI deployment faces in 2025–2026
                </p>
              </div>

              <div className="mt-3 w-full grid grid-cols-3 grid-rows-2 gap-3">
                {[
                  { num: "01", jp: "エージェント乗っ取り", en: "Prompt injection / agent goal hijacking",
                    body: "悪意あるメールや文書を読ませるだけで、AIが「攻撃者の指示」を自社の権限で実行してしまう攻撃。",
                    ev: <><strong className="font-bold">EchoLeak</strong> (MS Copilot, Jun 2025, CVSS 9.3) · <strong className="font-bold">ForcedLeak</strong> (Salesforce Agentforce, Sep 2025, CVSS 9.4)</> },
                  { num: "02", jp: "プロバイダーへの情報流出", en: "Cross-border data exposure to LLM providers",
                    body: "米国系AIへの入力は越境移転に該当（APPI第28条）。社員が無意識に顧客情報を貼り付けるケースが日常化。",
                    ev: <><strong className="font-bold">LayerX 2025:</strong> 77% of employees have pasted data into generative-AI tools as part of their work.</> },
                  { num: "03", jp: "サプライチェーン攻撃", en: "Malicious AI software packages",
                    body: "npm・PyPI・Docker等の部品に悪意あるコードを混入。1回の更新でAPIキーが全て盗まれる。",
                    ev: <><strong className="font-bold">April 2026:</strong> three simultaneous npm / PyPI / Docker Hub campaigns targeting AI tooling.</> },
                  { num: "04", jp: "APIキー流出・費用悪用", en: "Stolen API credentials → unauthorized usage",
                    body: "APIキーが流出すると一晩で数百万円の不正利用。鍵交換で止まるが、それまでの請求は残る。",
                    ev: <><strong className="font-bold">典型経路:</strong> GitHub誤コミット · 開発PC感染 · 社内Slack平文貼付</> },
                  { num: "05", jp: "シャドーAI（内部由来）", en: "Unauthorized personal AI accounts at work",
                    body: "社員が個人ChatGPTで業務データを処理。会社は何が外に出たか把握できない。DLPも効かない。",
                    ev: <><strong className="font-bold">2023 Samsung:</strong> engineers pasted source code → industry-wide ban precedent.</> },
                ].map((t) => (
                  <div key={t.num} className="rounded-xl p-3.5 border border-rose-200/70 bg-rose-50/70 flex flex-col">
                    <p className="text-[9px] font-bold uppercase tracking-widest text-[#8b5360] mb-1">脅威 {t.num} · Threat {t.num}</p>
                    <p className="text-base font-bold text-[#1D1D1F] leading-tight">{t.jp}</p>
                    <p className="text-[10px] text-[#86868B] italic leading-snug mt-0.5 mb-2">{t.en}</p>
                    <p className="text-[11px] text-[#5a4448] leading-relaxed flex-1">{t.body}</p>
                    <p className="text-[10px] text-[#8b5360] mt-2 pt-2 border-t border-rose-200/70 leading-snug">{t.ev}</p>
                  </div>
                ))}

                {/* Bridge tile */}
                <div className="rounded-xl p-3.5 border border-[#B8860B]/30 bg-[#FBF6E7] flex flex-col items-center justify-center text-center">
                  <p className="text-base font-bold text-[#B8860B] leading-tight">この 5つすべてに、<br/>具体的な対応があります。</p>
                  <p className="text-[10px] text-[#86868B] italic mt-1.5 leading-snug">There is a concrete response to every one of these. → Next 3 slides.</p>
                </div>
              </div>
            </div>
          </section>
  ...
  subgraph VPS["さくらVPS · 国内 · ISMAP"]
    ...
    PG["PromptGuard 2<br/>+ NeMo Guardrails"]:::accent
    KMS["Vault<br/>四半期ローテーション"]:::accent
    Audit["📋 audit log stream"]:::dark
  end
  classDef pc fill:#F5F5F7,stroke:#1D1D1F,color:#1D1D1F
  classDef layer fill:#E6F2EF,stroke:#1B7D5A,color:#1D1D1F
  classDef accent fill:#FBF6E7,stroke:#B8860B,color:#1D1D1F
  classDef dark fill:#1D1D1F,stroke:#1D1D1F,color:#FBF6E7`}</div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[#1D1D1F]/80 leading-relaxed"><strong className="text-[#B8860B] font-bold">PromptGuard 2 + NeMo Guardrails</strong> (dual injection control) · per-agent service accounts + MCP allowlist · <strong className="text-[#B8860B] font-bold">NTT おまかせサイバーみまもり</strong> 24/7 SOC · on-prem open-source AI (Llama 3 / Qwen / Gemma) · Vault + quarterly key rotation</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pt-3 border-t border-[#E8E8ED]">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-[#86868B] mb-1">月額 · Monthly</p>
                      <p className="text-xl font-bold text-[#B8860B] leading-tight">¥30,000–50,000</p>
                      <p className="text-[10px] text-[#86868B] italic mt-0.5">+ initial setup (scoped)</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-[#86868B] mb-1">適用 · Suited to</p>
                      <p className="text-xs text-[#1D1D1F] leading-snug">顧客PII・財務・医療・公共調達</p>
                    </div>
                  </div>
                  <p className="text-[10px] text-[#86868B] italic">独立セキュリティ監査（¥500,000〜、スコープに応じて見積）も別途ご相談可能。</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="h-full flex flex-col items-center justify-center px-8">
            <div className="flex flex-col items-center max-w-3xl text-center">
              ...
              <h2 className="text-4xl sm:text-6xl font-bold leading-tight">
                <span className="gradient-text">実費パススルー。<br/>マージン無し。</span>
              </h2>
              <p className="text-base text-[#86868B] italic mt-5 leading-relaxed">
                All vendor costs are pass-through. MOTTO adds no margin on security tooling.
              </p>
              <p className="text-lg text-[#1D1D1F] mt-8 leading-relaxed">
                セキュリティは「商品」ではなく、構え。<br/>必要な分だけ、透明に。
              </p>
              <p className="text-sm text-[#86868B] italic mt-2">
                Security is a posture, not a product. As much as you need, transparently priced.
              </p>
            </div>
          </section>
